...
object AudioFileUtils {
    ...
    fun saveAudioToFile(
        ...
        return try {
            ...
            Timber.d("音频文件已保存: ${audioFile.absolutePath}, 大小: ${audioData.size} bytes")
            audioFile
        ...
        try {
            ...
            if (audioFiles != null && audioFiles.size > maxFiles) {
                ...
                filesToDelete.forEach { file ->
                    if (file.delete()) {
                        Timber.d("删除旧音频文件: ${file.name}")
                    }
                }
            }
            
        } catch (e: Exception) {
            Timber.e(e, "清理旧音频文件失败")
        }
    }
