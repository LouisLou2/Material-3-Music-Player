...
fun ArtistsTopBar(
    ...
    gridSize: Int,
    sortOrder: Pair<ArtistsSortOption, IsAscending>,
    onChangeSortOption: (Pair<ArtistsSortOption, IsAscending>) -> Unit,
    ...
    TopAppBar(
        title = {
            Text(text = "Artists", fontWeight = FontWeight.SemiBold)
        },
...
fun SortOptionDropdownMenu(
    visible: Boolean,
    sortOption: ArtistsSortOption,
    isAscending: IsAscending,
    onChangeSortCriteria: (ArtistsSortOption) -> Unit,
    ...
    DropdownMenu(expanded = visible, onDismissRequest = onDismissRequest) {
        ...
        DropdownMenuItem(
            text = { Text("Name") },
            onClick = { onChangeSortCriteria(ArtistsSortOption.NAME) },
            trailingIcon = {
                RadioButton(selected = sortOption == ArtistsSortOption.NAME, onClick = null)
            }
        )
        Spacer(modifier = Modifier.width(16.dp))
        DropdownMenuItem(
            text = { Text("Number of Songs") },
            onClick = { onChangeSortCriteria(ArtistsSortOption.NUMBER_OF_SONGS) },
            trailingIcon = {
                RadioButton(
                    selected = sortOption == ArtistsSortOption.NUMBER_OF_SONGS,
                    onClick = null
                )
            }
        )
